...
class DuudApp {
  ...
  private exportToTypeScript(): void {
    ...
    const code = this.generateAnimationCode(animation);
    this.downloadTextFile(code, `${animation.name.replace(/\s+/g, '-').toLowerCase()}-animation.ts`);
  }

  private generateAnimationCode(animation: Animation): string {
    const keyframesCode = animation.keyframes.map(kf => {
      const paramsEntries = Object.entries(kf.params)
        .map(([key, value]) => `      ${key}: ${typeof value === 'number' ? value.toFixed(4) : JSON.stringify(value)}`)
        .join(',\n');
...
${paramsEntries}
    ...
    const sanitizedName = animation.name.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase();

    return `// Generated by Duud - Stick Figure Animation Studio
// Animation: ${animation.name}

import { Animation } from './animator';

export const ${sanitizedName}_ANIMATION: Animation = {
  name: '${animation.name}',
